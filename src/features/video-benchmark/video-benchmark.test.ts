...
const sample=(candidateId:BenchmarkSample["candidateId"],id:string,cost:number):BenchmarkSample=>({id,fixtureId:id,candidateId,repeat:1,seed:1,expectedCostUsd:cost,actualCostUsd:cost,latencyMs:1000,taskId:id,inputPath:null,outputPath:`${id}.mp4`,technical:evaluateTechnical(media),human:null,humanScore:null,status:"COMPLETED",generationCount:1,quotaUsage:null,humanLaborRequired:candidateId==="meta_vibes",sourceKind:candidateId==="meta_vibes"?"OWNER_MANUAL_IMPORT":"PROVIDER_API",error:null});
describe("video-provider-benchmark-v1",()=>{
  it("forecasts the low-cost first stage before making calls",()=>{const stage=selectCandidates(STAGE_ONE_CANDIDATES);expect(forecastCost(stage,3,1)).toBe(3.66);expect(minimumForecastCost(stage,3,1)).toBe(2.46)});
