...
const sample=(candidateId:BenchmarkSample["candidateId"],id:string,cost:number):BenchmarkSample=>{const manual=candidateId==="meta_vibes"||candidateId==="google_flow_manual";return{id,fixtureId:id,candidateId,repeat:1,seed:1,expectedCostUsd:cost,actualCostUsd:cost,latencyMs:1000,taskId:id,inputPath:null,sourcePath:`${id}-source.mp4`,outputPath:`${id}.mp4`,sourceDurationSeconds:8,normalizedDurationSeconds:8,technical:evaluateTechnical(media),human:null,humanScore:null,status:"COMPLETED",generationCount:1,quotaUsage:null,humanLaborRequired:manual,sourceKind:manual?"OWNER_MANUAL_IMPORT":"PROVIDER_API",error:null}};
describe("video-provider-benchmark-v1",()=>{
  it("forecasts the three-call fal primary stage before making calls",()=>{const stage=selectCandidates(STAGE_ONE_CANDIDATES);expect(forecastCost(stage,3,1)).toBe(.3);expect(minimumForecastCost(stage,3,1)).toBe(.3)});
