...
export async function importManualMetaClip(input:{sourcePath:string;outputPath:string;fixtureId:string;repeat?:number}){
  if(!existsSync(input.sourcePath))throw new Error("Owner-supplied Meta/Vibes clip was not found");if(existsSync(input.outputPath))throw new Error("Normalized Meta/Vibes output already exists; choose another output path");
  const renderer=new FFmpegVideoRenderer(),source=await renderer.probe(input.sourcePath);if(source.duration<7.88)throw new Error("Meta/Vibes source must contain at least eight seconds of video");const ffmpeg=binary();if(!ffmpeg)throw new Error("FFmpeg binary is unavailable");await mkdir(dirname(input.outputPath),{recursive:true});
  ...
  const sample:BenchmarkSample={id:`meta_vibes:${input.fixtureId}:${repeat}`,fixtureId:input.fixtureId,candidateId:"meta_vibes",repeat,seed:0,expectedCostUsd:0,actualCostUsd:0,latencyMs:Date.now()-started,taskId:null,inputPath:input.sourcePath,outputPath:input.outputPath,technical:evaluateTechnical(media),human:null,humanScore:null,status:"COMPLETED",generationCount:1,quotaUsage:null,humanLaborRequired:true,sourceKind:"OWNER_MANUAL_IMPORT",error:null};return {sample,sourceMedia:source,normalizedMedia:media,originalPreserved:true as const};
}
