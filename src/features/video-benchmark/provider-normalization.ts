...
export async function normalizeProviderBenchmarkClip(sourcePath:string,normalizedPath:string,options:{preserveAudio?:boolean}={}){
  ...
  await run(ffmpeg,["-n","-i",sourcePath,"-map","0:v:0",...(options.preserveAudio?["-map","0:a:0?"]:[]),"-t","8","-vf","scale=720:1280:force_original_aspect_ratio=decrease,pad=720:1280:(ow-iw)/2:(oh-ih)/2:black,fps=30,format=yuv420p","-c:v","libx264","-preset","medium","-crf","20",...(options.preserveAudio?["-c:a","aac"]:["-an"]),"-movflags","+faststart",normalizedPath]);
