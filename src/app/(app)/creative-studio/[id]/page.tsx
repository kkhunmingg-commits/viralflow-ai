...
import {createVideoFromCreativeAction} from "../../video-factory/actions";
...
export default async function CreativeProjectPage({params}:{params:Promise<{id:string}>}){
  ...
  const {project,context,angles,scripts,generations}=detail,scriptByAngle=new Map(scripts.map(s=>[s.creative_angle_id,s]));
  return <><PageHeading eyebrow="CREATIVE PROJECT" title={context.product.title} description={`${context.account.name} · ${context.account.mode} · Assignment ${context.assignment.score.toFixed(2)}`} action={<Link className="secondary-action" href="/creative-studio">← Creative Studio</Link>}/>
    ...
    <div className="creative-generate"><form action={generateCreativeAction}><input type="hidden" name="projectId" value={id}/><button className="primary-action">{angles.length?"REGENERATE 5 CONCEPTS":"GENERATE 5 CREATIVES"}</button></form>{project.selected_script_id?<form action={createVideoFromCreativeAction}><input type="hidden" name="projectId" value={id}/><button className="primary-action">CREATE VIDEO</button></form>:<span className="muted">เลือก concept ก่อนสร้างวิดีโอ</span>}</div>
