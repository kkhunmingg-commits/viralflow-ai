...
export default async function VideoFactoryPage({searchParams}:{searchParams:Promise<{page?:string}>}){
  const client=await createClient(),{data}=await client.auth.getUser();if(!data.user)redirect("/login");
  const page=parseOperationalPage((await searchParams).page),{items:masters,hasMore}=await listVideoFactory(client,data.user.id,page),plans=[createVideoCostPlan("CURRENT_3_ACCOUNTS"),createVideoCostPlan("FUTURE_10_ACCOUNTS")];
  return <><PageHeading eyebrow="MASTER → VARIATIONS" title="Video Factory" description="วิดีโอ commerce แนวตั้ง 8 วินาทีจาก Creative Brain ด้วยเส้นทางต้นทุนต่ำสุด"/>
    ...
    </Link>)}</section>:<section className="empty-state"><span>VF</span><h2>ยังไม่มี Master Video ในหน้านี้</h2><p>เลือกแนวคิดใน Creative Studio แล้วกด CREATE VIDEO เพื่อสร้าง MP4 ด้วย FFmpeg</p><Link className="primary-action" href="/creative-studio">ไปที่ Creative Studio</Link></section>}
